...
export async function sendAlertEmail(
  to: string, pageName: string, alerts: AlertItem[], pageId?: string, cards?: AiDiagCard[] | null,
  ...
  const rows = useCards
    ? cards!.map(c => `
      <tr><td style="padding:10px 14px;border-bottom:1px solid #eee;font-size:14px;line-height:1.5;color:#1f2937">
        ...
        <br/><span style="font-size:12px;color:#6b7280">建議：${c.cta.label}</span>
      </td></tr>`).join('')
    : alerts.map(a => `
      <tr><td style="padding:10px 14px;border-bottom:1px solid #eee;font-size:14px;line-height:1.5;color:#1f2937">
  ...
  <div style="font-family:-apple-system,'Helvetica Neue',Arial,sans-serif;max-width:560px;margin:0 auto">
    <h2 style="font-size:18px;color:#111827;margin-bottom:4px">📊 成效診斷優化建議 — ${pageName}</h2>
    <p style="font-size:13px;color:#6b7280;margin-top:0">為你整理了 ${count} 項廣告與內容的優化建議：</p>
    ...
    <p style="font-size:11px;color:#9ca3af;margin-top:24px">ContentLoop · 你可在設定頁調整通知頻率或關閉</p>
  </div>`

  const firstTitle = useCards ? cards![0].title : alerts[0].title
  const subject = count === 1
    ? `[ContentLoop] ${pageName} 成效診斷優化建議：${firstTitle.slice(0, 30)}`
    : `[ContentLoop] ${pageName} 成效診斷優化建議（${count} 項）`
