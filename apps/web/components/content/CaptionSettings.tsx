    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="mb-4 flex gap-2">
          <button onClick={() => setLanguage('zh')} className={chip(language === 'zh')}>{L('中文', 'Chinese')}</button>
          <button onClick={() => setLanguage('en')} className={chip(language === 'en')}>{L('英文', 'English')}</button>
        </div>
        ...
        {/* CTA（與目標一致） */}
        <label className="mb-1 block text-sm font-semibold text-gray-700">{L('CTA（與目標一致）', 'CTA (aligned to goal)')}</label>
        <div className="mb-2 flex flex-wrap gap-2">
