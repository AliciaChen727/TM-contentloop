...
### 🚨 第 0 步（最重要）：先確認廣告帳號歸在哪個商家組合
**做任何事之前**，先在右上角商家選單確認「**你實際投放的廣告帳號掛在哪個商家資產管理組合（Business Portfolio）底下**」，整個流程都要在**那個商家情境**下做。

- 已知坑（2026-06-13）：`D67 Toastmasters` 商家組合是 **0 項商家資產 / 「沒有可用的廣告帳號」**；實際投放帳號 `act_329347941177339` 掛在 **Legacy Toastmasters Club** 底下。
- 若在「沒有廣告帳號」的商家情境（如 D67）下建資料集 → 之後**沒有廣告帳號可共用 → ROAS 永遠歸不到**。
- 怎麼確認：右上角商家選單 → 各商家組合會顯示「N 個商家資產」，點進去看哪個有**廣告帳號**；或廣告儀表板現在有抓到數據的那個帳號 = 正確帳號，它所屬的商家組合就是要選的。
- **網站資料集必須建在「擁有該廣告帳號的商家組合」底下，並共用給該廣告帳號。**
